...
public:
    ...
    Dist(int N, int DIM, const std::string& D_DIR, const std::string& D_FILE){
        std::string D_PATH = D_DIR + "/" + D_FILE;
        std::ifstream d_file(D_PATH);
        if (!d_file.is_open()) {
            std::cerr << "Error: could not open file " << D_PATH << std::endl;
            exit(EXIT_FAILURE);
        }
        x.zeros(DIM, N); 
        P.zeros(N);
        ...
        std::string line;
        std::getline(d_file, line); // skip label line
        for (int i = 0; i < N; ++i) { // read distribution
...
==============================================================================*/
void record_data(const std::string& filename, const MC p, double t){
...
void RK87(MC& p, vec (*f)(vec, double, vec), double t0, double t1, double dt, Traj T, int DIM){
...
mat weighted_cov(const arma::mat& X, const arma::vec& w) {
    rowvec mean = trans(trans(X) * w);
    arma::mat centered_X = X.each_row() - mean;
    arma::mat cov = (centered_X.t() * (centered_X.each_col() % w)) / (1 - arma::dot(w, w));
...
void measurement_update(MC& p, vec (*h)(vec, vec), Meas M, Dist D, Traj T){
    ...
    // vec Ps = p.P % likelihoods; 
    // Ps /= sum(Ps);

    // arma::uvec indices_to_remove = arma::find(Ps < 1E-7);
    // p.x.shed_cols(indices_to_remove);
    // p.P.shed_rows(indices_to_remove);
    // p.P /= sum(p.P);

    // mat xn(p.dim, p.n, arma::fill::zeros);
    // vec Pn(p.n, arma::fill::zeros); 
    // double h2 = ((pow(4.0/(p.dim + 2.0), 2.0/(p.dim + 4.0)))*(pow(p.n, (-2.0/(p.dim + 4.0)))));
    // vec zero_mean(p.dim, arma::fill::zeros);
    // mat noise_cov = h2 * weighted_cov(p.x.t(), p.P); 
    // mat noise = arma::mvnrnd(zero_mean, noise_cov, p.n);

    // vec cdf(p.P.n_elem);
    // cdf(0) = p.P(0);
    // for (int i = 1; i < p.P.n_elem; ++i) {
    //     cdf(i) = cdf(i - 1) + p.P(i);
    // }
    ...
    //     index = std::min(index, static_cast<int>(cdf.size() - 1));
    //     xn.col(i) = p.x.col(index) + noise.col(i); 
    //     Pn(i) = p.P(index) * arma::as_scalar(exp(-0.5*trans(h(xn.col(i), T.coef)-M.mean)*arma::inv(M.cov)*(h(xn.col(i), T.coef)-M.mean)));
    // }
    // Pn = Pn/arma::sum(Pn);
    ...
    double h2 = ((pow(4.0/(p.dim + 2.0), 2.0/(p.dim + 4.0)))*(pow(D.n, (-2.0/(p.dim + 4.0)))));
    vec zero_mean(p.dim, arma::fill::zeros);
    mat noise_cov = h2 * weighted_cov(D.x.t(), D.P); 
    mat noise = arma::mvnrnd(zero_mean, noise_cov, p.n);
    ...
    for (int i = 0; i < p.n; ++i) {
        ...
        int index = std::distance(cdf.begin(), it);
        index = std::min(index, static_cast<int>(cdf.size() - 1));
        p.x.col(i) = D.x.col(index) + noise.col(i); // Add some noise to this at some point
        p.P(i) = arma::as_scalar(exp(-0.5*trans(h(p.x.col(i), T.coef)-M.mean)*arma::inv(M.cov)*(h(p.x.col(i), T.coef)-M.mean)));
    }
    p.P = p.P/arma::sum(p.P);

    return;
}

void run_mc(vec (*f)(vec, double, vec), vec (*h)(vec, vec), int* Dn, int N, int DIM_f, int DIM_h, mat Q, Meas M, Traj T, std::string P_DIR, std::string M_DIR, std::string D_DIR, int NUM_DIST, int NUM_MEAS, bool OUTPUT, bool RECORD, bool MEASURE){
    ...
    for(int nm = 0; nm < NUM_MEAS; nm++){
        printf("Timestep: %d-0, Program time: %f s, Sim. time: %f TU, Size: %d/%d\n", nm, ((double)(clock()-start))/CLOCKS_PER_SEC, tt, p.n, p.n); 
        ...
        for(int i=1; i < tspan.n_elem; i++){
            ...
            if (OUTPUT) { // print size to terminal
                printf("Timestep: %d-%d, Program time: %f s, Sim. time: %f TU, Size: %d/%d\n", nm, step_count, ((double)(clock()-start))/CLOCKS_PER_SEC, tt + tspan(i), p.n, p.n); 
            }
        ...
        if (!OUTPUT){ // print size to terminal
            printf("Timestep: %d-%d, Program time: %f s, Sim. time: %f TU, Size: %d/%d\n", nm, step_count - 1, ((double)(clock()-start))/CLOCKS_PER_SEC, tt, p.n, p.n); 
        }
        ...
        if ((MEASURE) && (nm < NUM_MEAS - 1)) { // peform discrete measurement update
            ...
            std::string D_FILE = "distribution"+ std::to_string(nm + 1) + ".txt";
            M = Meas(DIM_h, M_DIR, M_FILE);      
            Dist D = Dist(Dn[nm], DIM_f, D_DIR, D_FILE); 

            // performing discrete update
            measurement_update(p, h, M, D, T);                                
        }
    }
