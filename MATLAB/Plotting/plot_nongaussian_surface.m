...
for i=isovalue
    if (count == 1)&&p.display
        isosurface(X_grid, Y_grid, Z_grid, P_full, i, 'DisplayName', p.name); alpha(p.alpha(count));
    else
        isosurface(X_grid, Y_grid, Z_grid, P_full, i, 'HandleVisibility', 'off'); alpha(p.alpha(count));
    end
    count = count + 1; 
end
colormap(cool)