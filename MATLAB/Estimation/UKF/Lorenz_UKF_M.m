...
% Ben Hanson, 2024
%
% Applying the UKF to the Lorenz system, using built-in MATLAB function
...
xest0 = [-11.5; -10; 9.5]; Pest0 = diag([1,1,1]);
...
x  = NaN(d1,n1); x(:,1) = xest0;
xest = NaN(d1,n1); xest(:,1) = xest0 + sqrt(Q)*randn(d1,1); 
Pest = {}; Pest{1} = Pest0;
z  = NaN(d2,n2); count = 1;
...
for i = 2:n1
    ...
    if(mod(i-1,dt_ratio)==0)
        v = sqrt(R)*randn(d2,1);
        z(:,count) = h(x(:,i)) + v;  % True Measurement
...
[t, Y] = ode45(@(t, Y) Lorenz3D(Y,const), [0,50], xest0, options);
figure(2); 
...
plot3(xest(1,:),xest(2,:),xest(3,:),"c--",'LineWidth',1,"DisplayName","Estimated Trajectory");
scatter3(x(1,cad),x(2,cad),x(3,cad),'m', 'filled','HandleVisibility','off');
scatter3(xest(1,cad),xest(2,cad),xest(3,cad),'c', 'filled','HandleVisibility','off');

for i=round(linspace(1,length(tspan_x),6))
    plot_gaussian_ellipsoid(xest(1:3,i),Pest{i},3);
end 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
